  ...
  if (hsosHH == "Yes") {
    if("label::english" %in% colnames(koboQues)){
      ...
      exTableArabic <- koboQues %>% 
        select(type,relevant,name,`label::arabic`,`hint::arabic`,`constraint_message::arabic`) %>% 
...
paperFormArabic <- paperFormArabic %>% 
  mutate(`Choices Arabic` = ifelse(is.na(`Choices Arabic`) == T, "____________________",
                                    ifelse(Type == "note", "",`Choices Arabic`)))
...
noteStyle <- createStyle(fontName = "Arial Narrow", fontSize = 16,
                         halign = "center", fgFill = "#EE5757", 
                         textDecoration = "bold", border = c("top","bottom","left","right"), 
                         borderColour = "black", borderStyle = "thin")
...
noteSectionAra <- paperFormArabic %>% 
  rowid_to_column() %>% 
  filter(Type == "note")

noteSectionListAra <- noteSectionAra$rowid
...
if (relevancySelection == "Included"){
    ...
    ##Merge Arabic notes
    for (i in noteSectionListAra){
      mergeCells(wb, sheet = "paperArabic", rows = i + 2, cols = 1:5)
    }
    
    for(i in 1:nrow(noteSectionAra)){
      writeData(wb, sheet = "paperArabic", noteSectionAra$Name[i], startCol = 1, startRow = sum(noteSectionAra$rowid[i] + 2))
    }
    ...
    #English
    for (i in noteSectionListEng){
       addStyle(wb, sheet = "paperEnglish", noteStyle, rows = i + 2, cols = 1:5, gridExpand = F)
    }
    
    #Arabic
    for (i in noteSectionListAra){
       addStyle(wb, sheet = "paperArabic", noteStyle, rows = i + 2, cols = 1:5, gridExpand = F)
    }
    ...
    #English
    for (i in noteSectionListEng){
       addStyle(wb, sheet = "paperEnglish", noteStyle, rows = i + 2, cols = 1:4, gridExpand = F)
    }
    
    #Arabic
    for (i in noteSectionListAra){
       addStyle(wb, sheet = "paperArabic", noteStyle, rows = i + 2, cols = 1:4, gridExpand = F)
    }
